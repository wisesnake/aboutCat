            ...
            <c:choose>
              ...
              <c:otherwise>
				...
				<td width=50% align=left >
				  ...
				  <strong>주문자 번화번호:${item.orderer_phone}</strong><br>
				  <strong>수령자:${item.receiver_name}</strong><br>
				  <strong>수령자 번화번호:${item.receiver_phone}</strong><br>
				  <strong>주문상품명(수량):${item.goods_name}(${item.order_goods_qty})</strong><br>
				     <c:forEach var="item2" items="${orderList}" varStatus="j">
				       <c:if test="${j.index > i.index }" >
				          <c:if  test="${item.order_id ==item2.order_id}" >
				            <strong>주문상품명(수량):${item2.goods_name}(${item2.order_goods_qty})</strong><br>
				      </c:if>   
				       </c:if>
				    </c:forEach> 
				</td>
         ...
         <tr>
             <td colspan=8 class="fixed">
		         ...
		         <c:if test="${section >1 && page==1 }">
		          <a href="${contextPath}/admin/order/adminOrderMain.do?section=${section-1}&pageNum=${(section-1)*10 +1 }">prev&nbsp;</a>
		         </c:if>
		         <c:if test="${page == pageNum }">
		          <a href="${contextPath}/admin/order/adminOrderMain.do?section=${section}&pageNum=${page}"><Strong>${(section-1)*10 +page }</Strong> </a>
		         </c:if>
   		         <c:if test="${page != pageNum }">
